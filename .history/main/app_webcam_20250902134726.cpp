...
void WebCam::send_frame(uint8_t *buf, size_t len)
{
    ...
    void send_box(const char *url, int x_pos, int y_pos);

void WebCam::send_box(const char *url, int x_pos, int y_pos) {
    const char *TAG = "WebCam";
    ...
    esp_http_client_config_t config = {
        .url = "http://192.168.1.177:5000/box",
    ...
    esp_http_client_set_post_field(client, json_buf, len);

    // Send request
    esp_err_t err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        int status = esp_http_client_get_status_code(client);
        ESP_LOGI(TAG, "Bounding box sent. HTTP Status = %d", status);
    } else {
        ESP_LOGE(TAG, "HTTP request failed: %s", esp_err_to_name(err));
    }
...
void test_web_cam(){
    ...
    while (1) {
        ...
        detection->detectFace(web_cam.rgb888_image_data, fb->width, fb->height);
