...
void tDisplay::draw_number_circle(int x, int y, int r, int number) {
  // Clear the buffer to start fresh
  u8g2_ClearBuffer(&u8g2);

  // Draw a circle at the specified position with the given radius
  u8g2_DrawCircle(&u8g2, x, y, r, U8G2_DRAW_ALL);

  // Prepare the number to be displayed as a string
  char numStr[12]; // Enough to hold a number and a null terminator
  snprintf(numStr, sizeof(numStr), "%d", number); // Convert number to string

  // Choose a position to start drawing the number; let's position it centrally below the circle
  int text_x = x - (u8g2_GetStrWidth(&u8g2, numStr) / 2); // Center the text horizontally under the circle
  int text_y = y + r + u8g2_GetMaxCharHeight(&u8g2); // Position the text below the circle

  // Set the font for text rendering; choose an appropriate font for the size of your display
  u8g2_SetFont(&u8g2, u8g2_font_ncenB08_tr); // Choose a font; adjust size/style as necessary

  // Draw the string at the calculated position
  u8g2_DrawStr(&u8g2, text_x, text_y, numStr);
