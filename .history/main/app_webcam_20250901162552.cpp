...
{
    esp_http_client_config_t config = {0};     // wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
...
void test_web_cam(){
    ...
    ESP_ERROR_CHECK(web_cam.init_camera());
    uint8_t *rgb888_image_data = NULL;
    ...
    while (1) {
        ...
        if (!fb) {
           ...
           if (rgb888_image_data == NULL) {
               rgb888_image_data = (uint8_t *)malloc(fb->width * fb->height * 3);
               if (!rgb888_image_data) {
                   ESP_LOGE(TAG, "Failed to allocate memory for RGB888 image");
                   break;
               }
           }
        convertRGB565toRGB888(fb->buf, rgb888_image_data, fb->width, fb->height);

        send_frame(rgb888_image_data, fb->width * fb->height * 3);  // send_frame(rgb888_image_data);
